...
float stdev_double(double data[], int len) {
  double sum = 0;
  float mean;
  float SD = 0.0;
  int i;
...
float stdev_int(int data[], int len) {
  int sum = 0;
  float mean;
  float SD = 0.0;
  int i;
...
void assign_zones(region_t *region_list, int nregion, int nzone) {
    ...
    for (zid = 0; zid < nzone; zid++){
        for(rid = intervalnum*zid; rid < intervalnum*(zid+1); rid++){
          tempweights[zid] += (double)region_list[rid].node_count;
    ...
    int interval_stdev = stdev_double(tempweights, nzone);
    int greedy_stdev = stdev_int(zones, nzone);
